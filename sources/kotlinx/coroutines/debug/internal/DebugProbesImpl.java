...
public final class DebugProbesImpl {
    ...
    private final Function1<Boolean, Unit> getDynamicAttach() {
        ...
        try {
            ...
            m254constructorimpl = Result.m254constructorimpl((Function1) TypeIntrinsics.beforeCheckcastToFunctionOfArity(newInstance, 1));
        } catch (Throwable th) {
            Result.Companion companion2 = Result.INSTANCE;
            m254constructorimpl = Result.m254constructorimpl(ResultKt.createFailure(th));
        }
        return (Function1) (Result.m260isFailureimpl(m254constructorimpl) ? null : m254constructorimpl);
    }
    ...
    private final void updateRunningState(CoroutineStackFrame r5, String state) {
        boolean z;
        if (isInstalled$kotlinx_coroutines_core()) {
            ConcurrentWeakMap<CoroutineStackFrame, DebugCoroutineInfoImpl> concurrentWeakMap = callerInfoCache;
            DebugCoroutineInfoImpl remove = concurrentWeakMap.remove(r5);
            if (remove != null) {
                z = false;
            } else {
                CoroutineOwner<?> owner = owner(r5);
            ...
            Intrinsics.checkNotNull(r5, "null cannot be cast to non-null type kotlin.coroutines.Continuation<*>");
            remove.updateState$kotlinx_coroutines_core(state, (Continuation) r5, z);
            CoroutineStackFrame realCaller2 = realCaller(r5);
            if (realCaller2 == null) {
                return;
            }
    ...
    private final void updateState(CoroutineOwner<?> owner, Continuation<?> r3, String state) {
        if (isInstalled$kotlinx_coroutines_core()) {
            owner.info.updateState$kotlinx_coroutines_core(state, r3, true);
        }
    }
    ...
    private final <T> Continuation<T> createOwner(Continuation<? super T> completion, StackTraceFrame r6) {
        ...
        CoroutineOwner<?> coroutineOwner = new CoroutineOwner<>(completion, new DebugCoroutineInfoImpl(context, r6, atomicLongFieldUpdater.incrementAndGet(sequenceNumber$kotlinx$VolatileWrapper)));
