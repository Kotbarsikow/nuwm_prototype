...
final class DecodeInputStream extends InputStream {
    ...
    @Override // java.io.InputStream
    public int read(byte[] r10, int r11, int length) {
        int i;
        boolean z;
        boolean z2;
        Intrinsics.checkNotNullParameter(r10, "destination");
        if (r11 < 0 || length < 0 || (i = r11 + length) > r10.length) {
            throw new IndexOutOfBoundsException("offset: " + r11 + ", length: " + length + ", buffer size: " + r10.length);
        }
        if (this.isClosed) {
        ...
        if (getByteBufferLength() >= length) {
            copyByteBufferInto(r10, r11, length);
            return length;
        }
        ...
        while (true) {
            ...
            byteBufferLength -= i3;
            i2 += decodeSymbolBufferInto(r10, i2, i, i3);
        }
        if (i2 == r11 && z) {
            return -1;
        }
