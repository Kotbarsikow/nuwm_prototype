...
final class EncodeOutputStream extends OutputStream {
    ...
    @Override // java.io.OutputStream
    public void write(byte[] source, int r6, int length) {
        int i;
        Intrinsics.checkNotNullParameter(source, "source");
        checkOpen();
        if (r6 < 0 || length < 0 || (i = r6 + length) > source.length) {
            throw new IndexOutOfBoundsException("offset: " + r6 + ", length: " + length + ", source size: " + source.length);
        }
        if (length == 0) {
            return;
        }
        ...
        if (i2 != 0) {
            r6 += copyIntoByteBuffer(source, r6, i);
            if (this.byteBufferLength != 0) {
                return;
            }
        }
        while (r6 + 3 <= i) {
            int min = Math.min((this.base64.getIsMimeScheme() ? this.lineLength : this.symbolBuffer.length) / 4, (i - r6) / 3);
            int i3 = (min * 3) + r6;
            if (encodeIntoOutput(source, r6, i3) != min * 4) {
                throw new IllegalStateException("Check failed.".toString());
            }
            r6 = i3;
        }
        ArraysKt.copyInto(source, this.byteBuffer, 0, r6, i);
        this.byteBufferLength = i - r6;
    }
