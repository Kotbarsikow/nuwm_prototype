...
public final class Graphs {
    ...
    private static class TransposedGraph<N> extends ForwardingGraph<N> {
        ...
        class AnonymousClass1 extends IncidentEdgeSet<N> {
            ...
            /* renamed from: com.google.common.graph.Graphs$TransposedGraph$1$1 */
            class C00391 implements Function<EndpointPair<N>, EndpointPair<N>> {
                C00391() {
                }
        ...
        for (N n : mutableValueGraph.nodes()) {
            for (N n2 : valueGraph.successors((ValueGraph<N, V>) n)) {
                if (mutableValueGraph.nodes().contains(n2)) {
                    mutableValueGraph.putEdgeValue(n, n2, j$.util.Objects.requireNonNull(valueGraph.edgeValueOrDefault(n, n2, null)));
                }
            }
        }
    ...
    public static <N, V> MutableValueGraph<N, V> copyOf(ValueGraph<N, V> valueGraph) {
        ...
        for (EndpointPair<N> endpointPair : valueGraph.edges()) {
            standardMutableValueGraph.putEdgeValue(endpointPair.nodeU(), endpointPair.nodeV(), j$.util.Objects.requireNonNull(valueGraph.edgeValueOrDefault(endpointPair.nodeU(), endpointPair.nodeV(), null)));
        }
