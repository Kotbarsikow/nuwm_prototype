...
public class GoogleSignInButton extends AppCompatButton {
    ...
    private void parseAttributes(Context context, AttributeSet attributeSet, int i) {
        ...
        try {
            try {
                this.mText = obtainStyledAttributes.getString(R.styleable.ButtonStyleable_android_text);
                this.mIsDarkTheme = obtainStyledAttributes.getBoolean(R.styleable.ButtonStyleable_isDarkTheme, false);
            } catch (Exception e) {
                e.printStackTrace();
            }
        } finally {
