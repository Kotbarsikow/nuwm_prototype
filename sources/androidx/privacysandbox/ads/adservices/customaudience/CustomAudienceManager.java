    ...
    private static final class Api33Ext4Impl extends CustomAudienceManager {
        ...
        */
        ...
        private final android.adservices.customaudience.CustomAudience convertCustomAudience(CustomAudience request) {
            ...
            activationTime = new CustomAudience.Builder().setActivationTime(TimeConversions.convert(request.getActivationTime()));
            expirationTime = activationTime.setAds(convertAdData(request.getAds())).setBiddingLogicUri(request.getBiddingLogicUri()).setBuyer(convertAdTechIdentifier(request.getBuyer())).setDailyUpdateUri(request.getDailyUpdateUri()).setExpirationTime(TimeConversions.convert(request.getExpirationTime()));
            android.adservices.customaudience.CustomAudience build = expirationTime.setName(request.getName()).setTrustedBiddingData(convertTrustedSignals(request.getTrustedBiddingSignals())).setUserBiddingSignals(convertBiddingSignals(request.getUserBiddingSignals())).build();
            Intrinsics.checkNotNullExpressionValue(build, "Builder()\n              …\n                .build()");
            return build;
        }
