    ...
    private static final class Api33Ext5Impl extends MeasurementManager {
        ...
        */
        ...
        public final android.adservices.measurement.DeletionRequest convertDeletionRequest(DeletionRequest request) {
            ...
            start = new DeletionRequest.Builder().setDeletionMode(request.getDeletionMode()).setMatchBehavior(request.getMatchBehavior()).setStart(TimeConversions.convert(request.getStart()));
            end = start.setEnd(TimeConversions.convert(request.getEnd()));
            android.adservices.measurement.DeletionRequest build = end.setDomainUris(request.getDomainUris()).setOriginUris(request.getOriginUris()).build();
